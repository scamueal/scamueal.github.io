...
function unlockColor(color) {
  document.querySelector(`.snake-color[data-color="${color}"]`).classList.remove('locked');
  unlockedColors.push(color);
}

function setSnakeColor(color) {
  if (unlockedColors.includes(color) && snakeColor!== color) {
    snakeColor = color;
    
    // Update visual snake color
    snake.style.backgroundColor = color; 
    
    // Make new snakes spawned the new color
    snake.spawnColor = color;
  }
}
*/